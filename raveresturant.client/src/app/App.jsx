...
const GALLERY_ITEMS = [
    { label: "Truffle Arancini", bg: "#1a1208", accent: "#c9a84c" },
    { label: "The Dining Room", bg: "#0d1a1a", accent: "#4cb8c9" },
    { label: "Wagyu Striploin", bg: "#1a0d0d", accent: "#c94c4c" },
    { label: "Bar & Cocktails", bg: "#0d0d1a", accent: "#8b4cc9" },
    { label: "Sea Bass Plate", bg: "#0a1a10", accent: "#4cc97a" },
    { label: "Dessert Tasting", bg: "#1a1208", accent: "#c97a4c" },
];
...
export default function App() {
    ...
    return (
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-link { cursor: pointer; font-family: 'Montserrat', sans-serif; font-weight: 300; font-size: 0.72rem; letter-spacing: 0.18em; text-transform: uppercase; color: #c9b89a; transition: color 0.3s; padding: 4px 0; border-bottom: 1px solid transparent; }
        .nav-link:hover, .nav-link.active { color: #c9a84c; border-bottom-color: #c9a84c; }
        .btn-gold { display: inline-block; padding: 14px 40px; border: 1px solid #c9a84c; color: #c9a84c; font-family: 'Montserrat', sans-serif; font-size: 0.7rem; letter-spacing: 0.22em; text-transform: uppercase; cursor: pointer; background: transparent; transition: all 0.35s; }
        .btn-gold:hover { background: #c9a84c; color: #0c0a08; }
        .btn-ghost { display: inline-block; padding: 14px 40px; border: 1px solid rgba(240,235,227,0.3); color: #f0ebe3; font-family: 'Montserrat', sans-serif; font-size: 0.7rem; letter-spacing: 0.22em; text-transform: uppercase; cursor: pointer; background: transparent; transition: all 0.35s; }
        .btn-ghost:hover { background: rgba(240,235,227,0.08); }
        .menu-tab { font-family: 'Montserrat', sans-serif; font-size: 0.68rem; letter-spacing: 0.2em; text-transform: uppercase; padding: 10px 28px; border: 1px solid transparent; cursor: pointer; transition: all 0.3s; color: #8a7a68; }
        .menu-tab.active { border-color: #c9a84c; color: #c9a84c; }
        .menu-tab:hover { color: #c9a84c; }
        .menu-item { border-bottom: 1px solid rgba(201,168,76,0.12); padding: 24px 0; transition: background 0.3s; }
        .menu-item:hover { background: rgba(201,168,76,0.03); }
        .gallery-item { position: relative; overflow: hidden; cursor: pointer; }
        .gallery-item::after { content: ''; position: absolute; inset: 0; background: rgba(12,10,8,0.4); transition: opacity 0.4s; }
        .gallery-item:hover::after { opacity: 0; }
        .gallery-label { position: absolute; bottom: 16px; left: 20px; z-index: 2; font-family: 'Montserrat', sans-serif; font-size: 0.65rem; letter-spacing: 0.2em; text-transform: uppercase; color: rgba(240,235,227,0.7); transition: color 0.3s; }
        .gallery-item:hover .gallery-label { color: #c9a84c; }
        .form-field { background: transparent; border: none; border-bottom: 1px solid rgba(201,168,76,0.3); color: #f0ebe3; font-family: 'Cormorant Garamond', serif; font-size: 1rem; padding: 12px 0; width: 100%; outline: none; transition: border-color 0.3s; }
        .form-field:focus { border-bottom-color: #c9a84c; }
        .form-field::placeholder { color: #5a4e40; font-style: italic; }
        .divider { width: 60px; height: 1px; background: #c9a84c; }
        .section-label { font-family: 'Montserrat', sans-serif; font-size: 0.62rem; letter-spacing: 0.3em; text-transform: uppercase; color: #c9a84c; margin-bottom: 12px; }
        @keyframes float { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-8px); } }
        @keyframes fadeSlideUp { from { opacity:0; transform:translateY(40px); } to { opacity:1; transform:translateY(0); } }
        @keyframes lineExpand { from { width: 0; } to { width: 60px; } }
        .hero-title { animation: fadeSlideUp 1.2s ease 0.3s both; }
        .hero-sub { animation: fadeSlideUp 1.2s ease 0.6s both; }
        .hero-btns { animation: fadeSlideUp 1.2s ease 0.9s both; }
        .hamburger { display: none; flex-direction: column; gap: 5px; cursor: pointer; }
        .hamburger span { display: block; width: 24px; height: 1px; background: #c9a84c; transition: all 0.3s; }
        @media (max-width: 768px) {
          .hamburger { display: flex; }
          .nav-links { display: none; position: fixed; top: 0; right: 0; height: 100vh; width: 70vw; background: #0c0a08; padding: 80px 40px; flex-direction: column; gap: 28px; z-index: 99; transform: translateX(100%); transition: transform 0.4s ease; }
      ...
      `}</style>
            ...
            <nav style={{
                ...
                <div style={{ cursor: "pointer" }} onClick={() => scrollTo("Home")}>
                    <div style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, fontSize: "1.6rem", letterSpacing: "0.12em", color: "#f0ebe3" }}>RAVE</div>
                    <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "0.55rem", letterSpacing: "0.4em", color: "#c9a84c", textTransform: "uppercase", marginTop: "-2px" }}>Restaurant</div>
                </div>
                    ...
                    <FadeIn>
                        <div>
                            ...
                            <p style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 300, fontSize: "0.88rem", lineHeight: 1.9, color: "#8a7a68", marginBottom: "40px" }}>
                                Our team of passionate chefs, led by Head Chef Arjun Shrestha, transforms the finest seasonal ingredients into plates that are as beautiful as they are delicious. Every dish is a celebration of technique, tradition, and creativity.
                            </p>
            ...
            <footer style={{ padding: "60px 5% 40px", borderTop: "1px solid rgba(201,168,76,0.15)", textAlign: "center" }}>
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, fontSize: "2rem", letterSpacing: "0.12em", color: "#f0ebe3", marginBottom: "4px" }}>RAVE</div>
                <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "0.55rem", letterSpacing: "0.4em", color: "#c9a84c", textTransform: "uppercase", marginBottom: "32px" }}>Restaurant</div>
                ...
                <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "0.65rem", color: "#3a3028", letterSpacing: "0.08em" }}>
                    © 2026 Rave Restaurant, Kathmandu. All rights reserved.
                </div>
            </footer>
        </div>
    );
